...
export const setResult = async (req: Request, res: Response) => {
    try {
        ...
        res.status(201).json({ success: true, message: "Result Saved", Result: result });
...
export const getUserResult = async(req: Request, res: Response) => {
    try {
        ...
        const UserResult = await Result.aggregate([
            {
                $match: { user: userObjectId } // Match using ObjectId
            }
        ]);
        res.status(200).json({
            success: true,
